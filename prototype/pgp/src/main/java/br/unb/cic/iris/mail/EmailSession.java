...
public class EmailSession implements ConnectionListener {
		...
		if(getProvider().isAuthenticationEnabled()) {
			return Session.getInstance(props, new Authenticator() {
					...
					protected PasswordAuthentication getPasswordAuthentication() {
						return new PasswordAuthentication(getProvider().getUsername(),
	...
	protected final void connect(Service service, String host, int port) throws
	MessagingException {
		System.out.println("Connecting ...");
		service.connect(host, port, getProvider().getUsername(),
			getProvider().getPassword());
	}
