...
public class GmailProvider implements EmailProvider {
	...
	static {
		...
		properties.put(String.format("mail.%s.starttls.enable", transportProtocol),
			String.valueOf(transportAuth));
		properties.put(String.format("mail.%s.host", storeProtocol),
			"imap.gmail.com");
		properties.put(String.format("mail.%s.port", storeProtocol), "993");
		properties.setProperty(String.format("mail.%s.socketFactory.class",
				storeProtocol), "javax.net.ssl.SSLSocketFactory");
		...
		properties.setProperty(String.format("mail.%s.socketFactory.port",
				storeProtocol), String.valueOf("993"));
	}
