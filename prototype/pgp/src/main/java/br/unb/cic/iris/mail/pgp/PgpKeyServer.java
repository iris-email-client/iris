...
import static br.unb.cic.iris.core.SystemFacade.IRIS_HOME;
...
public class PgpKeyServer {
	...
	//private static PublicKey PublicKey publicKeyX509;
	...
	public static void main(String[] args) {
		String PUBLIC_FILE = IRIS_HOME + File.separator + "publicKeystore.pkr";
		char[] FILE_SECRET = "12345678".toCharArray();
		String identity = "teste_iris2@gmail.com";

		
		try {
			PgpKeyManager keyGenerator = new PgpKeyManager();
			PgpKeyServer server = new PgpKeyServer();
			// gera as chaves em arquivo
			//keyGenerator.generateKeys(identity);
			// le a chave publica do arquivo
			//String key = readFile(PUBLIC_FILE);
			//System.out.println("KEY=\n"+key);
			//server.teste(PUBLIC_FILE);
...
//			/server.send3(key);
			...
			//convert string retrieved into a pgp key
			PGPPublicKey publicKey = PgpUtils.readPublicKey(new ByteArrayInputStream(pubKeyStr.getBytes()));
			System.out.println("chave="+publicKey);
			//convert pgp key into x509
			PublicKey publicKeyX509 = new JcaPGPKeyConverter().getPublicKey(publicKey);
			System.out.println("PublicKey="+publicKeyX509);
